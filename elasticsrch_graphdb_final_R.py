...
es = Elasticsearch('10.208.23.165', port=9200, timeout=None)
#client = MongoClient('mongodb://'+MongoDbServer+':27017/')
client = MongoClient('10.237.26.159', 27017, j=True)
db = client['media-db'] 
mongo_coll='graphDbErTrial_R'
...
curl -X PUT '10.208.23.165:9200/index-temp-05feb20_r?pretty' -H 'Content-Type: application/json' -d'
{
    "settings" : {
        "index" : {
            "number_of_shards" : 5,
...
es_index='index-temp-05feb20_k'
es_mapping='mapping-temp-05feb20_k'
'''
create new mapping -- Rohit, 19 Jul 19
curl -X PUT '10.208.23.165:9200/index-temp-05feb20_r/_mapping/mapping-temp-05feb20_r?include_type_name=true' -H 'Content-Type: application/json' -d '{"properties": {}}'
...
def namesExactlyMatched_R(nameArr1, nameArr2):
    for n1 in nameArr1:
        for n2 in nameArr2:
        	bool1, bool2 = matchExact_R(n1, n2)
        	if bool1:
        		return True, True
...
def insertEntity(entity):
    ...
    try:
        ...
        print("")
    print('Inserting '+entity['stdName'])
    ...
    for relEn in relEntities:
        ...
        try:
            if entity.get('graphid', None) != None and relEn.get('graphid', None) != None:
                ...
                print("not done 2 G-Ids and 1 M-Id")
                continue
            # if entity.get('graphid', None)==None and relEn.get('graphid', None)==None:
                # continue
            bool1, bool2 = namesExactlyMatched_R(relEn['aliases'], changedProperties['aliases'])
            print(str(bool1)+' <> '+str(bool2))
            if bool1 and match_title_assoc_entity(relEn, changedProperties, relEn_assoc, cp_assoc):
            	index=1
            	changedProperties = getChangedProperties(changedProperties, relEn)
            	matchedEntities.append(relEn)
            elif not bool1 and bool2:
            	if (namesMatched(relEn['aliases'], changedProperties['aliases'])):
...
if __name__ == '__main__':
    collection = db['entities_resolved_graphdb_person_final']
    #collection = db['smallEntitySetModiCollection']
    ...
    count = 136499#0#136499#86297
    cursor = collection.find({'$and':[{'type':{'$in':['Person', 'Company', 'Organization','Party','City','BusinessBody']}}]}, no_cursor_timeout=True).skip(count)
    #count = 1
    ...
    for entity in cursor:
        ...
        if entity['type'] == 'Person':
            try:
                ...
                print('At line 948 Could not resolve '+entity['stdName']+' due to cursor error')
                cursor.close()
                time.sleep(600)
                cursor = collection.find({'$and':[{'type':{'$in':['Person', 'Company', 'Organization','Party','City','BusinessBody']}}]}, no_cursor_timeout=True).skip(count)
                continue  
