...
es = Elasticsearch('10.208.23.165', port=9200, timeout=30)
...
EScommand = 'systemctl restart elasticsearch.service'

'''
get status of elasticsearch service
sudo /etc/init.d/elasticsearch status
'''

'''
create new index -- Rohit, 19 Jul 19
curl -X PUT '10.208.23.165:9200/index-temp-3feb20?pretty' -H 'Content-Type: application/json' -d'
{
    "settings" : {
        "index" : {
            "number_of_shards" : 5,
...
es_index='index-temp-3feb20'# to change -- Rohit

'''
create new mapping -- Rohit, 19 Jul 19
curl -X PUT '10.208.23.165:9200/index-temp-3feb20/_mapping/mapping-temp-3feb20?include_type_name=true' -H 'Content-Type: application/json' -d '{"properties": {}}'

curl -X PUT '10.208.23.165:9200/index-temp-3feb20/_mapping/mapping-temp-3feb20?pretty' -H 'Content-Type: application/json' -d '{"properties": {}}'
'''
es_mapping='mapping-temp-3feb20'# to change -- Rohit

'''
create new collection and copy data into it from another  -- Rohit, 19 Jul 19
create mongodb collection
db.createCollection("mycollection")

saving subset of collection to another collection
db.getCollection('Test_19_Jul_19').insert(db.getCollection('unresolved_collection_20_Jun_19').find({'stdName':/Modi/i}).toArray())
'''
...
def checkSubString(string, sub_str): #inserted -- Rohit, 18 Jul 19
	'''
	returns True if substring is a subset of string
	'''
...
def abbreviateName(s):#inserted -- Rohit, 18 Jul 19
	'''
	return initials of provided string in lower case
	e.g. for s = "Narendra D Modi", funtion will return "ndm"
	'''
	l = s.split()
	abbr = ""
	for i in range(len(l)):
		s = l[i]
		abbr += s[0].lower()
	return ''.join(sorted(abbr))

def matchExact_R(name1, name2):#inserted -- Rohit, 18 Jul 19
    '''
    function for exact matching names
    returns two bools:- bool1, bool2
    for an exact name matching, bool1 and bool2 both are true
    for initials matching, bool1 is false and bool2 is true
    if there is no match, both bool1 and bool2 are false
    '''
    name1 = re.sub('\W+',' ',name1.lower())
    name2 = re.sub('\W+',' ',name2.lower())
    name1=name1.strip()
    ...
    if (name1 == name2):
		...
		abbrName1 = abbreviateName(name1)
		abbrName2 = abbreviateName(name2)
		    
		print('splitnames1: '+str(abbrName1))
		print('splitnames2: '+str(abbrName2))
		
		if(checkSubString(abbrName1,abbrName2) or checkSubString(abbrName2,abbrName1)):
			return False, True
		else:
			return False, False
...
def namesMatched_m(nameArr1, nameArr2):
    ...
    for n1 in nameArr1:
         n2=n1
         n3 = n2.encode('utf8')
         #print('yoyo : '+str(n3)+'  substring  '+str(l))
...
def namesExactlyMatched(nameArr1, nameArr2):
    for n1 in nameArr1:
        for n2 in nameArr2:
			bool1, bool2 = matchExact_R(n1, n2)#changed matchExact to matchExact_R -- Rohit, 18 Jul 19
			if bool1:
				return True, True
...
def getRelevantEntities(esQuery, mongo_coll):
    ...
    try:
    	...
    	traceback.print_exc()
    	print('in getRelevantEntities')
    	time.sleep(600)
        os.system('echo %s|sudo -S %s' % (sudoPassword, EScommand))
        #res = es.search(index=es_index, body=esQuery, doc_type=es_mapping)
        getRelevantEntities(esQuery, mongo_coll)
...
def resolvePersonEntity(entity,ent_to_hightfidf, mongo_coll):
    ...
    print('relEntities: '+str(relEntities))
    for relEn in relEntities:
        ...
        #print('relEn_assoc: '+str(relEn_assoc))
        #print('cp_assoc: '+str(cp_assoc))
        print('----------------------------------------------')
        ...
        try:
			bool1, bool2 = namesExactlyMatched(relEn['aliases'], changedProperties['aliases'])
			...
			if bool1:
				...
				print('in not bool1 and bool')
				if((namesMatched_m(relEn['aliases'], changedProperties['aliases']) and namesMatched_v(relEn['aliases'], changedProperties['aliases']))):
					index = 1
					changedProperties = getChangedProperties(changedProperties, relEn)
					matchedEntities.append(relEn)
				'''
    ...
    if index == 0:
        ...
        print(str(changedProperties['stdName']+' -- '+str(relEn['stdName'])))
...
def resolve(collection, ent_to_hightfidf, mongo_coll, cursor, chunk, chunk_size):
    ...
    for entity in cursor:
        try:
            ...
            if entity['type'] == 'Person':
                print('_____________________________________START_____________________________________________')
                print ('Resolving: '+entity['stdName']+' ObjectId: '+str(entity['_id']))
                resolvePersonEntity(entity,ent_to_hightfidf, mongo_coll)
                print('_____________________________________STOP______________________________________________')
            ...
            totalCount+=1
            print("Time taken: %.2fs\n"%(calculate_time_elapsed()))
        except (ConnectionError, connection.ConnectionError, connection.HTTPConnection, exceptions.ConnectionError):
            with open('./erLog.txt', 'a') as logFile:
                logFile.write(str(datetime.datetime.now())+'\n')
                logFile.write('-->ConnectionError\n')
                logFile.write('------------------------------------------------------\n\n')
            time.sleep(600)
            os.system('echo %s|sudo -S %s' % (sudoPassword, EScommand))
            continue
        except (errors.CursorNotFound, errors.ServerSelectionTimeoutError, errors.AutoReconnect):
            #traceback.print_exc()
            #print ('Stopped at '+str(datetime.now())+' for '+str(sys.exc_info()))
            with open('./erLog.txt', 'a') as logFile:
                logFile.write(str(datetime.datetime.now())+'\n')
                logFile.write('-->CursornotFound/ServerSelectionTimeoutError/AutoReconnect\n')
                logFile.write('Stuck at entity : '+str(count)+' for chunk : '+str(chunk)+'\n')
                logFile.write('------------------------------------------------------\n\n')
            cursor.close()
            time.sleep(600)#if there is an exception then wait for 10 minutes
            cursor = getCursor(collection, chunk, chunk_size, count)
            resolve(collection, ent_to_hightfidf, mongo_coll, cursor, chunk, chunk_size)
        except errors.NotMasterError:
        	#traceback.print_exc()
        	with open('./erLog.txt', 'a') as logFile:
        		logFile.write(str(datetime.datetime.now())+'\n')
        		logFile.write('-->NotMasterError\n')
        		logFile.write('------------------------------------------------------\n\n')
        	continue
        except:
            traceback.print_exc()
            #os.system('echo %s|sudo -S %s' % (sudoPassword, EScommand))
            #print('tried restarting elasticsearch')
            with open('./erLog.txt', 'a') as logFile:
                logFile.write(str(datetime.datetime.now())+'\n')
                logFile.write('-->AllOtherErrors\n')
                logFile.write('------------------------------------------------------\n\n')
            continue
...
def getCursor(collection, chunk, chunk_size, offset):
    '''
    Function to get a block of data accessed by a cursor
    if level0() calls it offset is 0
    if it is called incase of any exception offset must be number of entities that we have already covered
    '''
    cursor = collection.find({}, no_cursor_timeout=True).skip(chunk*chunk_size).limit(chunk_size)
    cursor.skip(offset).limit(chunk_size-offset)
    return cursor

def level0():
    #collection = db['46L_articles__unresolved_entities']
    coll_name ='UnresolvedEntities5000'# to change unresolved collection name --Rohit
    ...
    for chunk in range(0,1):
        ...
        #Rmongo_coll='chunk_%d_resolved_entities_r'%(chunk+1)
        mongo_coll='ResolvedEntities5000'# to change resolved collection name --Rohit
        
        #cursor = collection.find({}, no_cursor_timeout=True).skip(chunk*chunk_size).limit(chunk_size)
        cursor = getCursor(collection, chunk, chunk_size, 0)
        ent_to_hightfidf=getHighTFIDFDicts([cursor])
        #print(str(ent_to_hightfidf))
        cursor.close()
        print("Time taken: %.2fs\n"%(calculate_time_elapsed()))
        #print('cursor above: '+str(cursor)) -- inserted by Rohit, 17 Jul 19
        #cursor = collection.find({}, no_cursor_timeout=True).skip(chunk*chunk_size).limit(chunk_size)
        cursor = getCursor(collection, chunk, chunk_size, 0)
        #print('cursor BELOW: '+str(cursor)) -- inserted by Rohit, 17 Jul 19
        resolve(collection, ent_to_hightfidf, mongo_coll, cursor, chunk, chunk_size)
...
def levelPlus1():
    ...
    for chunk in range(1,16,2**level):
        ...
        unresolved_coll=db['chunk_%s_resolved_entities_r'%(name1)]
        unresolved_col2=db['chunk_%s_resolved_entities_r'%(name2)]
        resolved_col='chunk_%s_resolved_entities_r'%(name)
        print('chunk_%s_resolved_entities_r'%(name1))
        print('chunk_%s_resolved_entities_r'%(name2))
        print(resolved_col)
        ...
        resolve(unresolved_coll, ent_to_hightfidf, resolved_col, cursor, chunk, 1)
        cursor = unresolved_col2.find({}, no_cursor_timeout=True)
        resolve(unresolved_col2, ent_to_hightfidf, resolved_col, cursor, chunk, 1)
